...
from .middleware import request_response_formatter, unit_test_formatter
...
def data(*values):
    ...
    """
    if len(values) == 2:
        test_data = unit_test_formatter(values[0])
    elif values[0][-4:] == '.csv':
        test_data = request_response_formatter(values[0])
    else:
        test_data = values[0]
    global index_len
    index_len = len(str(len(test_data)))
    return idata(test_data)
